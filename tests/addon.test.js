...
test.concurrent.for(testCases)(
	'tmp $kind.type $variant',
	async (testCase, { page, ...ctx }) => {
		const cwd = ctx.cwd(testCase);

		const msg =
			"This is a text file made by the Community Addon Template demo for the add-on: 'tmp'!";

		const contentPath = path.resolve(cwd, `src/lib/tmp/content.txt`);
